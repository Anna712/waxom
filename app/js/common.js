...
$(window).scroll(function() {
  ...
  if (a == 0 && $(window).scrollTop() > oTop) {
	...
	$('.portfolio-list__item a').click(function() {
		...
		$('.item.project').unwrap();
	if(thisItem != 'All') {
		$('.item.project[data-category='+thisItem+']').wrapAll('<div class="row">').wrap('<div class="col-12 col-sm-6 col-md-4"></div>').slideDown(1000);
		$('.item.project[data-category!='+thisItem+']').slideUp(1000);
	} else {
		$('.item.project').wrapAll('<div class="row"></div>').wrap('<div class="col-12 col-sm-6 col-md-4"></div>').slideDown(1000);
	}
		return false;
	});
